...
class MessengerPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->busTag) as $busId => $tags) {
            ...
            if ($container->hasParameter($busMiddlewareParameter = $busId.'.middleware')) {
                ...
                $bag->remove($busMiddlewareParameter);
            }
    ...
    private function registerHandlers(ContainerBuilder $container, array $busIds)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->handlerTag) as $serviceId => $tags) {
            foreach ($tags as $tag) {
                ...
                $r = new \ReflectionClass($container->getParameterBag()->resolveValue($container->getDefinition($serviceId)->getClass()));
                ...
                foreach ($handles as $messageClass => $method) {
                    ...
                    if ('__invoke' !== $method) {
                        ...
                        $definitions[$definitionId = '.messenger.method_on_object_wrapper.'.self::hash($messageClass.':'.$messagePriority.':'.$serviceId.':'.$method)] = $wrapperDefinition;
                    } else {
        ...
        foreach ($handlersByBusAndMessage as $bus => $handlersByMessage) {
            foreach ($handlersByMessage as $message => $handlersIds) {
                if (1 === \count($handlersIds)) {
                    ...
                    $chainHandler->setPublic(false);
                    $serviceId = '.messenger.chain_handler.'.self::hash($bus.$message);
        ...
        foreach ($busIds as $bus) {
            $container->register($resolverName = "$bus.messenger.handler_resolver", ContainerHandlerLocator::class)
                ->setArguments([$this->registerServiceLocator($container, $handlersLocatorMappingByBus[$bus] ?? [])]);
    ...
    private function registerBusMiddleware(ContainerBuilder $container, string $busId, array $middlewareCollection)
    {
        ...
        foreach ($middlewareCollection as $middlewareItem) {
            ...
            if (($definition = $container->findDefinition($messengerMiddlewareId))->isAbstract()) {
                $childDefinition = new DefinitionDecorator($messengerMiddlewareId);
    ...
    private function registerServiceLocator(ContainerBuilder $container, $handlers)
    {
        $definition = new Definition(ServiceLocator::class, [$handlers]);
        $definition->setPublic(false);

        $locatorId = self::hash($definition);
        if (!$container->hasDefinition($locatorId)) {
            $container->setDefinition($locatorId, $definition);
        }
